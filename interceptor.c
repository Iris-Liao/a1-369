...
extern void* sys_call_table[];   //'extern'extend the visibility of function to the whole program;The system call table is exported

/* The sys_call_table is read-only => must make it RW before replacing a syscall */
void set_addr_rw(unsigned long addr) {

	unsigned int level;
	pte_t *pte = lookup_address(addr, &level);  //??pte_t
...
typedef struct {
	...
	asmlinkage long (*f)(struct pt_regs);//'asmlinkage'store all arguments on stack instead of in registers partly

	/* Status: 1=intercepted, 0=not intercepted */
	int intercepted;

	/* Are any PIDs being monitored for this syscall? */
	int monitored;  //0-no monitored;1-partly monitored;2-all pids are monitored by this syscall
...
/**
 * 1.Add a pid to a syscall's list of monitored pids.
...
/**
 * 2.Remove a pid from a system call's list of monitored pids.
...
/**
 * 3.Remove a pid from all the lists of monitored pids (for all intercepted syscalls).
...
/**
 * 4.Clear the list of monitored pids for a specific syscall.
 */
...
/**
 * 5.Check if two pids have the same owner - useful for checking if a pid
 ...
 * owner of that pid is allowed to request that.   //process cannot intercept process B's task
 */
...
/**
 * 6.Check if a pid is already being monitored for a specific syscall.
...
{
spin_lock(&pidlist_lock);//how to test synchronization???
del_pid(current->pid);
...
/**
 * 2.This is the generic interceptor function.
 ...
 *      For the first two commands, we must be root (see the current_uid() macro).//current_uid()!=0
 ...
 */
 //1.intercept function
 asmlinkage long my_syscall_intercept_function(int syscall, int pid)
 {
 	table[syscall].f=sys_call_table[syscall];//f is the function pointer used to save address of original syscall,sys_call_table[syscall] is the address where the system call function is put
 	table[syscall].intercepted=1;
 	spin_lock(&calltable_lock);
 	set_addr_rw((unsigned long)sys_call_table);//set syscall table writable
  sys_call_table[syscall]=&interceptor;//interceptor function
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	int isOwned;//if not owned:-EPERM;if owned-0
	int isMonitored;//check if the current pid has already been monitored
	int isAdded;//check if the pid can be added to list;
...
switch (cmd) {
	...
	if(table[syscall].intercepted==1)
			 return -EBUSY;
my_syscall_intercept_function(syscall,pid);
break;
  ...
  my_syscall_reslease_function(syscall,pid);
  break;
...
static int init_function(void) {
	 int i;
	 spin_lock_init(&calltable_lock); //initializations for spinlocks
